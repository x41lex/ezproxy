...
func createMockSpawner(t *testing.T) *MockSpawnerInfo {
	...
	ps, err := handler.NewProxySpawnerWithContainer(sAddr, cnt.Execute, ctx)
	if err != nil {
		t.Fatalf("Failed to create proxy spawner: %v", err)
		return nil
	}
	err = ps.RegisterMpx("TestMpx", handler.PxProtoAll, pAddr, ipl.Execute)
	if err != nil {
		t.Fatalf("Failed to add MPX: %v", err)
		return nil
	}
	<-ch
	doSetup = false
...
func TestNewProxyCancel(t *testing.T) {
	...
	cancel()
	_, err := handler.NewProxySpawner(sAddr, ctx)
	if err == nil {
		t.Fatalf("No error when creating proxy spawner")
	}
...
func TestNewProxy(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	ps, err := handler.NewProxySpawner(sAddr, ctx)
	if err != nil {
		t.Fatalf("Failed to create proxy spawner with listener: %v", err)
	}
	err = ps.RegisterMpx("testMpx", handler.PxProtoAll, pAddr, ipl.Execute)
	if err != nil {
		t.Fatalf("Failed to register testMpx: %v", err)
	}
...
func TestNewProxyMultiple(t *testing.T) {
	...
	sAddr := NewMockAddr("Server Net")
	pa1 := NewMockAddr("Proxy Net 1")
	pa2 := NewMockAddr("Proxy Net 2")
	pa3 := NewMockAddr("Proxy Net 3")
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	wg.Add(3)
	ps, err := handler.NewProxySpawner(sAddr, ctx)
	...
	err = ps.RegisterMpx("test1", handler.PxProtoAll, pa1, ipl1.Execute)
	if err != nil {
		t.Errorf("Failed to add 'test1' mpx: %v", err)
		wg.Done()
	}
	err = ps.RegisterMpx("test2", handler.PxProtoAll, pa2, ipl2.Execute)
	if err != nil {
		t.Errorf("Failed to add 'test2' mpx: %v", err)
		wg.Done()
	}
	err = ps.RegisterMpx("test3", handler.PxProtoAll, pa3, ipl3.Execute)
	if err != nil {
		t.Errorf("Failed to add 'test3' mpx: %v", err)
		wg.Done()
	}
	wg.Wait()
}
...
func TestGetAddrs(t *testing.T) {
	ms := createMockSpawner(t)
	defer ms.Close()
	pAddr, err := ms.Spawner.GetProxyAddr("TestMpx")
	if err != nil {
		t.Errorf("Failed to get proxy addr: %v", err)
	} else {
